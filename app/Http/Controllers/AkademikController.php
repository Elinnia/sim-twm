...
class AkademikController extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($this->user_type == "wali_kelas") {
            ...
            $data_nilai = Tb_Akademik::whereIn("kode_walikelas", $this->kode_walikelas)->groupBy("id_tahun_ajaran")->get();
            $id_tahun_ajaran = [];
            foreach ($data_nilai as $dn) {
                $id_tahun_ajaran[] = $dn->id_tahun_ajaran;
            }
            $data_nilai = Tb_Akademik::whereIn("kode_walikelas", $this->kode_walikelas)->groupBy("kode_jurusan")->get();
            $kode_jurusan = [];
            foreach ($data_nilai as $dn) {
        ...
        return view("pages/akademik/index")->with($param);
    }
    ...
    public function get_data(Request $request)
    {
        ...
        $dt = Tb_Siswa::where("kelas", $param['kelas'])
            ...
            ->get();
        foreach ($dt as &$d) {
            $dt_akademik = Tb_Akademik::where("nisn", $d->nisn)
            ...
            $kode_akademik = "";
            if ($dt_akademik == true) {
                $kode_akademik = $dt_akademik->kode_akademik;
                $keterangan = $dt_akademik->keterangan;
            }
            $d->kode_akademik = $kode_akademik;
    ...
    public function store(Request $request)
    {
        ...
        foreach ($nisn as $n) {
            ...
            $kode_akademik = $param['kode_akademik'][$i];
            if (strlen($kode_akademik) < 1) {
                ...
                Tb_Akademik::where("kode_akademik", $kode_akademik)->update($p);
            }
            $i++;
        }
    }
