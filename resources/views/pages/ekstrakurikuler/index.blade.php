...
<div class="card-body">
    <form class="form_eskul">
     <div class="row">
        ...
        <div class="col-sm-4">
           <div class="form-group">
            <label class="text-primary">Kegiatan Ekstrakurikuler</label>
            <select name="kegiatan_ekstrakurikuler" class="form-control select2 kegiatan_ekstrakurikuler">
                <option value="kegiatan_kepramukaan">Kegiatan Kepramukaan</option>
                <option value="kegiatan_seni_musik">Kegiatan Seni Musik</option>
                <option value="kegiatan_sepak_bola">Kegiatan Sepak Bola</option>
                <option value="kegiatan_pencaksilat">Kegiatan Pencaksilat</option>
                <option value="kegiatan_seni_tari">Kegiatan Seni Tari</option>
            </select>
          </div>
...
<!-- Custom styles for this page -->
 ...
 <!-- Page level plugins -->
    ...
    <script type="text/javascript">
        ...
        function get_data(){
            var form  = $(".form_eskul").serializeArray();
            var url = '{{route('ekstrakurikuler.get_data')}}';
            $.ajax({
                type:'POST',
                data:form,
                url:url,
                    ...
                    $.each(data,function(i,item){
                        html+=`<tr>
                            <td>
                                ...
                                <input type="hidden" name="kode_ekstrakurikuler[]" value="${item.kode_ekstrakurikuler}">
                            </td>
                            <td>${item.nama_peserta_didik}</td>
                            
                            <td><input type='text' class='form-control form-control-sm keterangan' name='keterangan[]' value='${item.keterangan}' placeholder="Keterangan Ekstrakurikuler" ></td>
                        </tr>`;
                    });
                    $("#tb_data").html(html);
                },
        ...
        function submit(){
            var form  = $(".form_eskul").serializeArray();
            var url = '{{route('ekstrakurikuler.store')}}';
            $.ajax({
                type:'POST',
                data:form,
                url:url,
                ...
                success:function(data){
                    ...
                    Swal.fire({
                      icon: 'success',
                      title: 'Pemberitahuan',
                      text: 'Ekstrakurikuler berhasil di input'
                    })
                    get_data();
                },
                error:function(data){
